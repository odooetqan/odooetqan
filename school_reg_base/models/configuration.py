...
class Discounts(models.Model):
    ...
    ratio = fields.Float("Ratio", tracking=True, required=True)
...
class TracksBase(models.Model):
    ...
    def create(self, vals):
        res = super(TracksBase, self).create(vals)
        analytic_account = self.env['account.analytic.account'].create({
            'name': res.display_name,
            'plan_id': self.env['account.analytic.plan'].create({'name': res.name}).id
        })
        res.analytic_account_id = analytic_account.id
        return res
     
    def write(self, vals):
        res = super(TracksBase, self).write(vals)
        if 'name' in vals:
            for rec in self:
...
class SectionsBase(models.Model):
    ...
    @api.depends('name', 'track_id.name')
    def _compute_display_name(self):
        for rec in self:
            name = rec.name or ' '
            track_name = rec.track_id.name or ' '
            rec.display_name = name + ' ' + track_name

    @api.model
    def create(self, vals):
        res = super(SectionsBase, self).create(vals)
        analytic_account = self.env['account.analytic.account'].create({
            'name': res.display_name,
    ...
    def write(self, vals):
        res = super(SectionsBase, self).write(vals)
        if 'name' in vals:
            for rec in self:
                rec.analytic_account_id.write({'name': rec.display_name})
        return res
...
class StagesBase(models.Model):
    ...
    @api.depends('name', 'section_id.name', 'track_id.name')
    def _compute_display_name(self):
        for rec in self:
            name = rec.name or ' '
            track_name = rec.track_id.name or ' '
            section_name = rec.section_id.name or ' '
            rec.display_name = name + ' ' + ' ' + section_name + ' ' + track_name
   
    @api.model
    def create(self, vals):
        res = super(StagesBase, self).create(vals)
        analytic_account = self.env['account.analytic.account'].create({
            'name': res.display_name,
            'plan_id': res.track_id.analytic_account_id.plan_id.id
        })
    ...
    def write(self, vals):
        res = super(StagesBase, self).write(vals)
        if 'name' in vals:
            for rec in self:
                rec.analytic_account_id.write({'name': rec.display_name})
        return res
...
class ClassesBase(models.Model):
    ...
    seq = fields.Char(string="Class Sequences")
    display_name= fields.Char(string="Display Name", compute = "_compute_display_name")
    product_id = fields.Many2one('product.template', string='Product')
    analytic_account_id = fields.Many2one('account.analytic.account', string='Anyltic Account')

    @api.depends('name', 'stage_id.name', 'section_id.name', 'track_id.name')
    def _compute_display_name(self):
        for rec in self:           
            name = rec.name or ' '
            track_name = rec.track_id.name or ' '
...
class Years(models.Model):
    _name = "years"
    _description = "Describe Years of students"

    name = fields.Integer("Name", tracking=True, required=True)
    company_current_year = fields.Integer(compute="_compute_company_current_year", store=True)
...
class Fees(models.Model):
    ...
    name = fields.Char("Name", tracking=True, required=True)
    stage_id = fields.Many2one("stages", tracking=True, required=True)
    product_id = fields.Many2one("product.template", tracking=True, required=True)
    analytic_account_id = fields.Many2one('account.analytic.account', string='Anyltic Account')
