...
class Partner(models.Model):
    ...
    def _check_name(self):
        ...
        if partner_rec:
            raise ValueError(_('مكرر  ! هذا العميل موجود من قبل في جهات الاتصال .'))
    @api.constrains('mobile')
    def _check_mobile(self):
        partner_rec = self.env['res.partner'].search(
            [('mobile', '=', self.mobile), ('id', '!=', self.id)]) 
        if partner_rec:
            raise ValueError(_('مكرر ! هذا الرقم موجود في احدي جهات الاتصال '))
    @api.constrains('id_number')
    def _check_mobile(self):
        partner_rec = self.env['res.partner'].search(
            [('id_number', '=', self.id_number), ('id', '!=', self.id)]) 
        if partner_rec:
            raise ValueError(_('مكرر ! هذا الهوية موجود في احدي جهات الاتصال '))
