...
# -----------------------------
# Extend res.partner correctly
# -----------------------------
class ResPartnerStudent(models.Model):
    _inherit = "res.partner"   # FIXED: was res.parnter (typo)

    device_id = fields.Many2one("biostar.device", string="الجهاز", readonly=True)


# -----------------------------
# Student Attendance base model
# -----------------------------
class StudentAttendance(models.Model):
    ...
    student_id = fields.Many2one(
        ...
        # keep this only if res.partner has student_number in your DB
        domain=[('student_number', '!=', False)]
    )

    # UNCOMMENT ONLY if these models exist & their module is installed
    # class_id  = fields.Many2one("classes.classe", string="الفصل")
    # group_id  = fields.Many2one(
    #     "classes.groups", string="الشعبة/المجموعة",
    ...
    # Helper used by device integration
    def _student_status_from_checkin(self, device, date_val, first_dt):
        ...
        if not first_dt:
            cutoff = start_dt + timedelta(minutes=cutoff_min)  # FIXED
...
# -----------------------------
# BioStar User (requires biostar.device)
# -----------------------------
