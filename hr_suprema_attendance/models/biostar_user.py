...
class StudentAttendance(models.Model):
    ...
    student_id = fields.Many2one(
        "res.partner", string="الطالب", required=True, index=True,
    )
    
    # domain=[('student_number', '!=', False)]

    # If you have these models installed, you can add them later:
    class_id  = fields.Many2one("classes.classe", string="الفصل")
    group_id  = fields.Many2one("classes.groups", string="الشعبة/المجموعة",
                               ) # domain="[('classe_id','=',class_id)]"
    ...
    attachment = fields.Binary(string="المرفق")
    ...
    # def _student_status_from_checkin(self, device, date_val, first_dt):
    #     """Arabic status from device windows."""
    #     start_float = getattr(device, 'start_attendance_student', 8.0) or 8.0
    #     grace       = int(getattr(device, 'time_to_present_late', 10) or 10)
    #     cutoff_min  = int(getattr(device, 'time_to_calculate_absent', 60) or 60)
