...
class BiostarDevice(models.Model):
    ...
    #     def _probe_exists():
    #         return bool(self._search_device_user(recc, uid))

    #     # fast-exit if already on device
    #     if _probe_exists():
    #         return True

    #     # Resolve group/partition: prefer device form values; else discover.
    #     gid = None
    #     pid = None
    #     if self.default_user_group_id not in (None, "", False):
    #         try:
    ...
    #         try:
    #             pid = int(self.default_partition_id)
    #         except Exception:
    #             pass
    ...
    #         if rows:
    #             g = next((r for r in rows if str(r.get("name", "")).lower() in {"all users", "default"}), rows[0])
    #             if g.get("id") is not None:
    #                 gid = int(g["id"])
    #             part = g.get("partition") or {}
    #             if pid is None and isinstance(part, dict) and part.get("id") is not None:
    #                 pid = int(part["id"])

    #     # --- 1) BULK shape to /api/users (your server accepts this) --------------
    #     # Matches your “best of creating multi users”.
    #     try:
    #         bulk_row = {
    #             "user_id": uid,
    #             "name": display,
    ...
    #         }
    #         if gid is not None:
    #             bulk_row["user_group_id"] = {"id": int(gid)}
    #         if pid is not None:
    #             bulk_row["partition"] = {"id": int(pid)}

    #         body = {"UserCollection": {"rows": [bulk_row]}}
    #         return recc._req("POST", "/users", json=body)
    #     except UserError as e_bulk:
    #         emsg = (e_bulk.args[0] or "").lower()
    #         if "already" in emsg or "exists" in emsg or _probe_exists():
    #             return True
    #         # if complaint mentions group/partition, we’ll try single-user with same shape
    #         group_problem = any(k in emsg for k in ("group", "partition", "site", "doesn't exist"))

    #     # --- 2) Single user v1 to /api/users (your “best of creating one user”) ---
    #     # Keep it minimal and accepted widely; avoid optional fields that some servers reject.
    #     try:
    #         single_user = {
    #             "User": {
    #                 "user_id": uid,
    ...
    #         }
    #         if gid is not None:
    #             single_user["User"]["user_group_id"] = {"id": int(gid)}
    #         if pid is not None:
    #             single_user["User"]["partition"] = {"id": int(pid)}

    #         return recc._req("POST", "/users", json=single_user)
    #     except UserError as e_single:
    #         smsg = (e_single.args[0] or "").lower()
    #         if "already" in smsg or "exists" in smsg or _probe_exists():
    #             return True
    #         # final bubble-up (shows the BioStar body to the Odoo user)
    #         raise
